...
def test_migrate_v2_to_v3(temp_vault_dir, policy_yaml, master_provider):
    ...
    conn.execute("INSERT INTO secrets (secret_id, namespace, name, version, ciphertext, checksum, created_at, updated_at, status, dek_version) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", ('sid1', 'default', 'old_secret', 1, ciphertext, 'chk', 'now', 'now', 'ACTIVE', 0))
    conn.commit()
    conn.close()
