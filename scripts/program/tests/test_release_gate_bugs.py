...
def test_release_gate_uses_list_response_directly(monkeypatch):
        ...
        return [
            {
                ...
                "status": "todo", "labels": [{"name": "pri:blocker"}],
            },
            {
                "id": "u-2", "identifier": "RGT-2", "project_id": "ms-1",
                "status": "done", "labels": [],
            },
        ]
    ...
    for name, _, _ in skipped:
        assert name in {
            "changelog_updated", "threat_model_current",
            "dependency_audit", "secret_scan", "sbom_generated",
        }
...
def test_dashboard_uses_list_response_directly(monkeypatch):
    ...
    fake_list_issues = mock.MagicMock(return_value=[
        {"id": "u-1", "identifier": "RGT-1", "project_id": "ms-1",
         "status": "todo", "labels": [], "priority": "medium"},
    ])
...
def test_wip_audit_uses_list_response_directly(monkeypatch):
    ...
    fake_list_issues = mock.MagicMock(return_value=[
        {
            "id": "u-1", "identifier": "RGT-1", "status": "todo",
            "labels": [{"name": "effort:M"}],
            "assignee_id": "m1", "assignee_type": "member",
