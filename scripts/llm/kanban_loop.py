...
def main() -> int:
    ...
    p.add_argument("--dry-run", action="store_true", help="plan the loop, don't call LLMs or post comments")
    p.add_argument("--once", action="store_true", help="run one pass and exit (default for cron use)")
    p.add_argument("--loop", action="store_true", help="keep running indefinitely (sleep between passes)")
    p.add_argument("--sleep", type=int, default=60, help="seconds between passes when --loop is set")
    p.add_argument("--limit", type=int, default=10, help="max tickets per pass")
    p.add_argument("--status", default="todo,in_progress", help="comma-separated statuses to include")
    p.add_argument("--milestone", default=None, help="filter to tickets in this milestone project (e.g. v0.2.0)")
    p.add_argument(
        "--state", default=str(DEFAULT_STATE_PATH), help="path to state file (default /tmp/rgt_kanban_loop_state.json)"
    )
    p.add_argument(
        "--reset-state", action="store_true", help="ignore previous state and re-process all in-scope tickets"
    )
    p.add_argument("--workspace-id", default=WORKSPACE_ID)
    p.add_argument("--max-tokens", type=int, default=600)
    p.add_argument("--timeout", type=int, default=180)
    p.add_argument("--quiet", action="store_true", help="suppress per-ticket output (only summary)")
    args = p.parse_args()
    ...
    while True:
        pass_count += 1
        if not args.quiet:
            print(f"\n=== PASS {pass_count} @ {time.strftime('%Y-%m-%d %H:%M:%S')} ===", file=sys.stderr)
        ...
        for i in selected:
            ...
            try:
                ...
                if not args.quiet:
                    print(f"  [{tid}] EXCEPTION: {type(e).__name__}: {e}", file=sys.stderr)
        ...
        if not args.quiet:
            print(
                f"\n=== PASS {pass_count} done. Processed {processed_this_pass} tickets. State: {state_path}",
                file=sys.stderr,
            )
...
def _process_one_ticket(issue_summary, tid, args, router, processed, state, state_path):
        ...
        if last_attempt:
            try:
                ...
                if age_min < cooldown_min:
                    if not args.quiet:
                        print(
                            f"  [skip-fail] {tid} failed {fail_info['count']}x, "
    ...
    if prev and not args.reset_state_run and not ticket_changed(full, prev.get("signature", "")):
        if not args.quiet:
            print(f"  [skip] {tid} already processed @ {prev.get('at', '?')}", file=sys.stderr)
        return
    ...
    if not args.quiet:
        print(f"\n  [{tid}] {task} via {pair.get('primary')} + {pair.get('secondary')}", file=sys.stderr)
    ...
    if not ok:
        ...
        if not args.quiet:
            ...
            print(
                f"  [{tid}] all models failed (attempt #{info['count']}), skipping comment post; cooldown {cooldown}m",
                file=sys.stderr,
            )
        return
    ...
    for spec, r in ok:
        lines.append(
            f"---\n**{spec}** ({r.provider}/{r.model}, {r.latency_ms}ms, in={r.input_tokens} out={r.output_tokens}):\n"
        )
        lines.append(r.text)
        lines.append("")
    if errs:
    ...
    if 200 <= status_code < 300:
        if not args.quiet:
            print(f"  [{tid}] posted synthesis (HTTP {status_code})", file=sys.stderr)
        processed[tid] = {
        ...
        if not args.quiet:
            print(f"  [{tid}] comment post failed: HTTP {status_code}: {resp[:200]}", file=sys.stderr)
